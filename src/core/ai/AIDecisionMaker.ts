...
export class AIDecisionMaker {
   ...
   */
  private generateForcePreservationDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
    ...
    for (const unit of context.availableUnits) {
      // Skip units already used in higher priority decisions
   ...
   */
  private generateObjectiveDefenseDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateTerrainDenialDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateCombatDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];

    // Find units that can attack and haven't been used yet
    ...
    for (const unit of combatUnits) {
      ...
      if (targets.length > 0) {
        ...
        for (const target of targets) {
          ...
          if (engagement.shouldEngage && engagement.confidence > 0.3) {
            ...
            // Boost priority for damaged enemies to encourage finishing them off
            const isDamaged = target.state.currentHP < target.stats.hp;
            const basePriority = 15;
            const damageBonus = isDamaged ? 5 : 0; // Extra priority for damaged targets

            decisions.push({
              type: AIDecisionType.ATTACK_TARGET,
              priority: basePriority + damageBonus, // Very high priority for actual combat, extra for damaged enemies
              unitId: unit.id,
      ...
      } else if (!unit.state.hasMoved && unit.stats.mv && unit.stats.mv > 0) {
        // Unit has no targets in range and can move - move toward nearest enemy!
   ...
   */
  private generateIntelligenceDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateWaspOperationsDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateHiddenOperationsDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateLogisticsDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateSpecialAbilityDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
    ...
    for (const unit of context.availableUnits) {
      if (unit.specialAbilities.length > 0 && unit.canAct() && !usedUnits.has(unit.id)) {
        // Check if unit has immediate combat opportunities
        const immediateTargets = this.findValidTargets(unit, context.enemyUnits, context);
        const hasImmediateCombat = immediateTargets.some(target => {
          const distance = this.calculateDistance(unit.state.position, target.state.position);
          return distance <= 1; // Adjacent or same hex
        });

        // Skip special abilities if unit should be fighting instead
        if (hasImmediateCombat) {
          // Don't generate special ability - let combat decision take priority
          continue;
        }
        ...
        // Lower priority if any enemies are nearby (within 2 hexes)
        const nearbyEnemies = context.enemyUnits.filter(enemy => {
          const distance = this.calculateDistance(unit.state.position, enemy.state.position);
          return distance <= 2;
        });

        const priority = nearbyEnemies.length > 0 ? 4 : 7; // Reduced priority near combat

        decisions.push({
          type: AIDecisionType.SPECIAL_ABILITY,
          priority: priority,
          unitId: unit.id,
          reasoning: hasImmediateCombat
            ? `Using special ability: ${abilityName} (no immediate combat)`
            : `Using special ability: ${abilityName}`,
          metadata: {
            abilityName: abilityName,
            combatAvailable: hasImmediateCombat,
   ...
   */
  private generateObjectiveSecuringDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
   ...
   */
  private generateCenterMovementDecisions(
    context: AIDecisionContext,
    usedUnits: Set<string>
  ): AIDecision[] {
    const decisions: AIDecision[] = [];
    ...
    for (const unit of context.availableUnits) {
      ...
      if (!unit.state.hasMoved && unit.stats.mv && unit.stats.mv > 0) {
