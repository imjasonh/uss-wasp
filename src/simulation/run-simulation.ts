...
function calculatePlayerScore(player: Player): number {
  const UNIT_SCORE_MULTIPLIER = 10;
  const CP_SCORE_MULTIPLIER = 5;
  const livingUnits = player.getLivingUnits();
  const unitScore = livingUnits.length * UNIT_SCORE_MULTIPLIER;
  const cpScore = player.commandPoints * CP_SCORE_MULTIPLIER;
 ...
 */
function runSingleGame(gameId: string, config: SimulationConfig): SimulationResult {
 ...
 */
function runSimulation(config: SimulationConfig = DEFAULT_CONFIG): SimulationSummary {
  ...
  for (let i = 0; i < config.gameCount; i++) {
    ...
    try {
      const result = runSingleGame(gameId, config);
      results.push(result);

      let winnerText: string;
      if (result.winner === PlayerSide.Assault) {
        winnerText = 'Assault';
      } else if (result.winner === PlayerSide.Defender) {
        winnerText = 'Defender';
      } else {
        winnerText = 'Draw';
      }
  ...
  const averageDefenderScore =
    results.reduce((sum, r) => sum + r.defenderScore, 0) / results.length;
...
function printSummary(summary: SimulationSummary): void {
  ...
  console.log(
    `Assault Wins: ${summary.assaultWins} (${((summary.assaultWins / summary.totalGames) * 100).toFixed(1)}%)`
  );
  console.log(
    `Defender Wins: ${summary.defenderWins} (${((summary.defenderWins / summary.totalGames) * 100).toFixed(1)}%)`
  );
  console.log(
...
function main(): void {
  ...
  const config: SimulationConfig = {
    ...
    aiDifficulty: (args[1] as AIDifficulty) ?? DEFAULT_CONFIG.aiDifficulty,
    logLevel: (args[2] as SimulationConfig['logLevel']) ?? DEFAULT_CONFIG.logLevel,
  };
