...
export class ComprehensiveAITest {
  ...
  private async runBasicCombatTest(): Promise<void> {
    ...
    try {
      ...
      const defenderUnits = createTestUnits([
        ...
        { id: 'defender-2', type: UnitType.ATGM_TEAM, side: PlayerSide.Defender, position: new Hex(7, 6) },
        { id: 'defender-3', type: UnitType.TECHNICAL, side: PlayerSide.Defender, position: new Hex(6, 7) }
      ]);
      ...
      assaultUnits.forEach(unit => {
        try {
          ...
          console.log(`✅ Added assault unit: ${unit.type} at (${unit.state.position.q}, ${unit.state.position.r})`);
        } catch (error) {
          testResult.errors.push(`Failed to add assault unit ${unit.id}: ${error instanceof Error ? error.message : String(error)}`);
        }
      });
      ...
      defenderUnits.forEach(unit => {
        try {
          ...
          console.log(`✅ Added defender unit: ${unit.type} at (${unit.state.position.q}, ${unit.state.position.r})`);
        } catch (error) {
          testResult.errors.push(`Failed to add defender unit ${unit.id}: ${error instanceof Error ? error.message : String(error)}`);
        }
      });
      ...
      try {
        ...
        testResult.errors.push(`Failed to add AI controllers: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
      }
      ...
      while (turnCount < maxTurns && !this.isGameOver(gameState)) {
        ...
        try {
          ...
          testResult.errors.push(`Turn ${turnCount} error: ${error instanceof Error ? error.message : String(error)}`);
          console.log(`❌ Turn ${turnCount} failed: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      ...
      testResult.errors.push(`Test setup failed: ${error instanceof Error ? error.message : String(error)}`);
      console.log(`❌ Basic Combat Test failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private async runAmphibiousAssaultTest(): Promise<void> {
    ...
    try {
      ...
      const assaultUnits = createTestUnits([
        { id: 'wasp-1', type: UnitType.USS_WASP, side: PlayerSide.Assault, position: new Hex(0, 4) },
        ...
        { id: 'harrier-1', type: UnitType.HARRIER, side: PlayerSide.Assault, position: new Hex(0, 3) },
        { id: 'cobra-1', type: UnitType.SUPER_COBRA, side: PlayerSide.Assault, position: new Hex(0, 5) }
      ]);

      // Create defender coastal defense
      const defenderUnits = createTestUnits([
        { id: 'inf-1', type: UnitType.INFANTRY_SQUAD, side: PlayerSide.Defender, position: new Hex(8, 3) },
        { id: 'inf-2', type: UnitType.INFANTRY_SQUAD, side: PlayerSide.Defender, position: new Hex(9, 4) },
        { id: 'inf-3', type: UnitType.INFANTRY_SQUAD, side: PlayerSide.Defender, position: new Hex(8, 6) },
        { id: 'atgm-1', type: UnitType.ATGM_TEAM, side: PlayerSide.Defender, position: new Hex(10, 4) },
        { id: 'aa-1', type: UnitType.AA_TEAM, side: PlayerSide.Defender, position: new Hex(9, 5) },
        { id: 'mortar-1', type: UnitType.MORTAR_TEAM, side: PlayerSide.Defender, position: new Hex(11, 5) },
        { id: 'artillery-1', type: UnitType.LONG_RANGE_ARTILLERY, side: PlayerSide.Defender, position: new Hex(11, 2) }
      ]);
      ...
      try {
        ...
        testResult.aiProgrammingGaps.push('AI decision making for USS Wasp operations not verified');
      } catch (error) {
        testResult.errors.push(`USS Wasp operations test failed: ${error instanceof Error ? error.message : String(error)}`);
      }
      ...
      while (turnCount < maxTurns) {
        ...
        try {
          ...
          testResult.errors.push(`Amphibious AI test turn ${turnCount}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      ...
      testResult.success = testResult.errors.length === 0;
      console.log(`${testResult.success ? '✅' : '❌'} Amphibious Assault Test ${testResult.success ? 'PASSED' : 'FAILED'}`);

    } catch (error) {
      testResult.errors.push(`Amphibious assault test failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private async runUSSWaspOperationsTest(): Promise<void> {
    ...
    try {
      ...
      const waspUnit = createTestUnit('wasp-1', UnitType.USS_WASP, PlayerSide.Assault, new Hex(0, 2));
      const harrierUnit = createTestUnit('harrier-1', UnitType.HARRIER, PlayerSide.Assault, new Hex(0, 2));
      const marineUnit = createTestUnit('marines-1', UnitType.MARINE_SQUAD, PlayerSide.Assault, new Hex(0, 2));
      ...
      try {
        ...
        testResult.gameEngineGaps.push('USS Wasp launch/recovery operations need automated testing');
        testResult.gameEngineGaps.push('USS Wasp cargo capacity management needs AI integration');
        testResult.aiProgrammingGaps.push('AI strategic planning for USS Wasp positioning needs verification');

      } catch (error) {
        testResult.errors.push(`USS Wasp AI test failed: ${error instanceof Error ? error.message : String(error)}`);
      }

      testResult.success = testResult.errors.length === 0;
      console.log(`${testResult.success ? '✅' : '❌'} USS Wasp Operations Test ${testResult.success ? 'PASSED' : 'FAILED'}`);

    } catch (error) {
      testResult.errors.push(`USS Wasp operations test setup failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private async runHiddenUnitTest(): Promise<void> {
    ...
    try {
      ...
      testResult.errors.push(`Hidden unit test failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private async runMultiDifficultyTest(): Promise<void> {
    ...
    try {
      ...
      const noviceUnits = createTestUnits([
        { id: 'novice-1', type: UnitType.MARINE_SQUAD, side: PlayerSide.Assault, position: new Hex(0, 0) },
        { id: 'novice-2', type: UnitType.MARINE_SQUAD, side: PlayerSide.Assault, position: new Hex(1, 0) }
      ]);

      const eliteUnits = createTestUnits([
        { id: 'elite-1', type: UnitType.INFANTRY_SQUAD, side: PlayerSide.Defender, position: new Hex(4, 4) },
        { id: 'elite-2', type: UnitType.ATGM_TEAM, side: PlayerSide.Defender, position: new Hex(5, 4) }
      ]);
      ...
      while (turnCount < maxTurns) {
        ...
        try {
          ...
          testResult.errors.push(`Multi-difficulty test turn ${turnCount}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      ...
      if (noviceActionCount === eliteActionCount) {
        testResult.aiProgrammingGaps.push('No difference in action count between NOVICE and ELITE AI');
      }

      testResult.performance = { turnCount, aiDecisionTime: 0, totalGameTime: 0 };
      testResult.success = testResult.errors.length === 0;
      
      console.log(`${testResult.success ? '✅' : '❌'} Multi-difficulty Test ${testResult.success ? 'PASSED' : 'FAILED'}`);

    } catch (error) {
      testResult.errors.push(`Multi-difficulty test failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private async runResourceManagementTest(): Promise<void> {
    ...
    try {
      ...
      testResult.aiProgrammingGaps.push('AI prioritization of high-cost vs low-cost actions needs testing');
      ...
      testResult.errors.push(`Resource management test failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  ...
  private getPhaseDescription(phase: number): string {
    const phases = [
      'Command Phase', 'Movement Phase', 'Combat Phase', 
      'Rally Phase', 'Resupply Phase', 'End Phase'
    ];
