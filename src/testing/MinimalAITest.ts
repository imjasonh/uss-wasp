...
export async function runMinimalAITest(): Promise<void> {
  ...
  try {
    ...
    console.log(`✅ Performance metrics initialized: ${status.performanceMetrics.casualtiesInflicted === 0}\n`);
    ...
    console.log(`✅ Assault AI: ${gameEngine.isAIControlled(assaultPlayer.id) ? 'Enabled' : 'Disabled'}`);
    console.log(`✅ Defender AI: ${gameEngine.isAIControlled(defenderPlayer.id) ? 'Enabled' : 'Disabled'}\n`);
    ...
    try {
      ...
      console.log(`ℹ️ AI processing with no units: ${error instanceof Error ? error.message : String(error)}\n`);
    }
    ...
    if (assaultAIStatus && defenderAIStatus) {
      console.log(`✅ Assault AI Status: ${assaultAIStatus.difficulty}, State: ${assaultAIStatus.currentState}`);
      console.log(`✅ Defender AI Status: ${defenderAIStatus.difficulty}, State: ${defenderAIStatus.currentState}`);
    } else {
